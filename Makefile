...
build_with_out = CGO_ENABLED=1 go build -tags frontend_packr -ldflags="-linkmode external -extldflags '-static' -s -w -X github.com/alufers/biedaprint/core.AppVersion=$(TRAVIS_TAG) -X github.com/alufers/biedaprint/core.AppReleaseExecutableName=$(1)" -o $(ROOT_DIR)/build/$(1) server/server.go
